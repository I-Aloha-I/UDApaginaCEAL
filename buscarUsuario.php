...
<body>
    ...
    <form class="form-register form-noModal" autocomplete="off" action="" method="POST">
        ...
        <input class="controls" type="text" name="busquedaRut" placeholder="Ingrese Run que desea buscar">
        <input class="controls" type="text" name="busquedaApellido" placeholder="Ingrese Apellidos que desea buscar">
        <!--BOTONES / LINKS-->
        <input class="botones" type="submit" name="buscar" value="BUSCAR">
        <div class="botones" style="height: 45px; text-align: center; font-size: 16px; line-height: 20px; margin-top: 0px"><a style="text-decoration: none" href="gestionUsuarios.php">VOLVER</a></div>
    </form>

    <!-------MOSTRANDO LOS DATOS------>
    <?php
        if(isset($_POST['buscar'])){
            $rut=$_POST['busquedaRut'];
            $apellido=$_POST['busquedaApellido'];

            // caso 1 - rut
            if(!empty($rut) and empty($apellido)){
                $sql = "SELECT * FROM usuarios WHERE Rut LIKE '%$rut%'";
                $query=mysqli_query($Conexion,$sql);
                include("tablaUsuarios.php");
            }
            // caso 2 - apellidos
            else if(!empty($apellido) and empty($rut)){
                $sql = "SELECT * FROM usuarios WHERE Apellido LIKE '%$apellido%'";
                $query=mysqli_query($Conexion,$sql);
                include("tablaUsuarios.php");
            }
            // caso 3 - rut y apellidos
            else if(!empty($rut) and !empty($apellido)){
                $sql = "SELECT * FROM usuarios WHERE Rut LIKE '%$rut%' and Apellido LIKE '%$apellido%'";
                $query=mysqli_query($Conexion,$sql);
                include("tablaUsuarios.php");
            }
            // caso 4 - nada
