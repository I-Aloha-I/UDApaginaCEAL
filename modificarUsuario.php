...
<body>
    ...
    <form class="form-register form-noModal" autocomplete="off" action="Actions/updateUsuario.php" method="POST">
        <h4>MODIFICACION DE USUARIOS</h4>
        <input class="controls" type="text" name="Rut" id="Rut" value="<?php echo $row['Rut'] ?>" placeholder="Ingrese su Run" pattern="[0-9]{7,8}-[0-9kK]{1}" Title="Ingrese su run, sin puntos y con guíon" required>
        <input class="controls" type="text" name="Nombre" id="Nombre" value="<?php echo $row['Nombre'] ?>" placeholder="Ingrese sus Nombres" pattern="[a-zA-ZñÑüÜáéíóúÁÉÍÓÚ ]{1,200}" Title="Solo puede usar letras, tildes y espacios" required>
        <input class="controls" type="text" name="Apellido" id="Apellido" value="<?php echo $row['Apellido'] ?>" placeholder="Ingrese sus Apellidos" pattern="[a-zA-ZñÑüÜáéíóúÁÉÍÓÚ ]{1,200}" Title="Solo puede usar letras, tildes y espacios" required>
        <input class="controls" type="email" name="Correo" id="Correo" value="<?php echo $row['Correo'] ?>" placeholder="Ingrese su Correo" pattern="[a-z]{1,100}.[a-z]{1,100}.[0-9]{1,2}@alumnos.uda.cl" Title="Debe ingresar su correo institucional" required>
        <input class="controls" type="password" name="Contra" id="Contra" value="<?php echo $row['Contra'] ?>" placeholder="Ingrese su Contraseña" pattern="[0-9a-zA-Z ]{3,50}" Title="Solo puede usar letras, números y espacios. La contraseña debe contener mínimo 5 caracteres" required>
        <!--Pregunta de seguridad-->
        <select class="controls" id="pregunta" name="pregunta" required>
        <option value="" disabled selected><?php echo $row['pregunta'] ?></option>
        ...
        <!--Respuesta-->
        <input type="text" value="<?php echo $row['respuesta'] ?>" placeholder="Ingrese una respuesta" id="respuesta" name="respuesta" class="controls" required>
